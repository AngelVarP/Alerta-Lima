...
const applyFilters = () => {
    router.get('/supervisor/denuncias', {
        search: search.value,
...
watch([selectedEstado, selectedAsignado], () => {
    applyFilters();
});
...
const hasActiveFilters = () => {
    return search.value || selectedEstado.value || selectedAsignado.value;
};
</script>

<template>
    <Head title="Supervisión de Denuncias" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-900 dark:text-white leading-tight">
                👁️ Supervisión de Denuncias del Área
            </h2>
        </template>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="relative">
                        ...
                        <input
                            ...
                            placeholder="Buscar por código, título o ciudadano..."
                        />
                    </div>
                ...
                <ul v-else class="divide-y divide-gray-200 dark:divide-gray-700">
                    <li v-for="denuncia in denuncias.data" :key="denuncia.id">
                        <Link
                            ...
                            class="block p-5 hover:bg-purple-50/50 dark:hover:bg-gray-700/50 transition duration-150"
                        >
                            <div class="flex items-start justify-between">
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400 flex-wrap">
                                        ...
                                        <span v-if="denuncia.asignado_a" class="flex items-center gap-1 font-medium text-purple-600 dark:text-purple-400">
                                            👤 {{ denuncia.asignado_a.nombre }}
                                        </span>
                                    </div>
                                </div>
